...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0">
    ...
    <xsl:template match="/">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="-24 -250 {count(//undertale)*(64)} {$maxHeight - 350}" width="100%" height="100%">
...
<!--                <line x1="0" y1="-286" x2="0" y2="0" stroke="purple" stroke-width="2"/>-->
                <xsl:for-each select="//undertale">
                    ...
                    <text x="{$xPosition + 18}" y="{$nums[$yPosition]}" text-anchor="middle" class="fileName label-text">
                        <xsl:value-of select="substring-before(@file, '.xml')"/>
                    </text>
                    <text class="label-text" x="{$xPosition + 18}" y="-{$lineCount + 5}" text-anchor="middle">
